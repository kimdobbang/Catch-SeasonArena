...
import com.catchcatch.auth.domains.rank.adapter.out.client.FindAllRankingClient;
...
@Slf4j(topic = "auth")
public class FindAllRankingController {

    private final ObjectMapper objectMapper;
    private final FindAllRankingClient findAllRankingClient;

    @GetMapping("/all/{page}")
    public ResponseEntity<?> findTierRanking(Authentication authentication, @PathVariable Integer page) {
        Member member = ((PrincipalDetails) authentication.getPrincipal()).getMember();
        try {
            ResponseEntity<Map<String, Object>> response = findAllRankingClient.findAllRanking(member.getEmail(), page);
