...
import com.catchcatch.auth.domains.inventory.adapter.out.UnEquipInventoryClient;
...
@RestController
@RequestMapping("/api/auth/inventories/items/unequipment")
@Slf4j(topic = "auth")
@RequiredArgsConstructor
public class UnEquipInventoryController {

	private final UnEquipInventoryClient unEquipInventoryClient;
	private final ObjectMapper objectMapper;

	@PutMapping("/{inventory_id}")
	public ResponseEntity<?> unEquipInventory(@PathVariable("inventory_id") Long inventoryId,
		Authentication authentication) {
		Member member = ((PrincipalDetails)authentication.getPrincipal()).getMember();
		try {
			ResponseEntity<Map<String, Object>> response = unEquipInventoryClient.equipInventory(inventoryId,
				member.getEmail());
